...
public class MainActivity extends FragmentActivity implements LoaderManager.LoaderCallbacks<List<Artist>>, SongsAdapter.SongSelectedListener,
        ColorsAdapter.AccentChangedListener, AlbumsAdapter.AlbumSelectedListener, ArtistsAdapter.ArtistSelectedListener {
    ...
    public void shareAudio(View v) {

        if (mPlayerAdapter != null && mPlayerAdapter.isMediaPlayer()) {
            Song currentSong = mPlayerAdapter.getCurrentSong();
            Album currentAlbum = mPlayerAdapter.getSelectedAlbum();

            if (currentSong != null && currentAlbum != null) {
                String filePath = currentSong.path;
                Log.d("filepath",filePath);
                String authority = "com.iven.musicplayergo.provider";
                if (filePath != null) {
                    ...
                    if (f.exists()) {
                      //  Uri uri = Uri.parse("file://" + f.getAbsolutePath());
                        ...
                        Toast.makeText(getApplicationContext(), "Song sharing successfully", Toast.LENGTH_SHORT).show();
                    }
                }
            }
        }
...
//    }
    ...
    public void openLinkedinPage(View v) {
        try {
            startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("https://www.linkedin.com/in/saurabh-bhosale-31b169252/")));
